...
int main(int argc, char **argv) {
  ...
  argp_init(&ap, "minspatree_seq", "1.1.0",
            "Minimum Spanning Tree with sequential algorithm.");
  ...
  for (int i = 0; i < n_edges; i++) {
    size_t result =
        fscanf(fp, "%d %d %ld", &edges[i].src, &edges[i].dst, &edges[i].weight);
    test(result == 3);
  }
  ...
  // --- E' = {}
  edge_t *E_line =
      malloc((n_vertices - 1) * sizeof(edge_t)); // Máximo de arestas na MST
  test(E_line != NULL);
