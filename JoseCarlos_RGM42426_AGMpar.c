...
int main( int argc, char **argv ) {
    ...
    while ( processed_edges < edges_len ) {
        logging( RANK, INFO, "Iniciando lote. Arestas processadas: %u / %u", processed_edges, edges_len );
    ...
    //=========================================================================
    if ( ROOT ) {
        logging( RANK, INFO, "Iniciando Borůvka sequencial no ROOT sobre %zu candidatos.", m );
        ...
        while ( !done ) {
            iteracao++;
            logging( RANK, INFO, "Iteracao %d. Componentes restantes: %zu", iteracao, components_len );
        ...
        logging( RANK, INFO, "Concluida. MST final possui %zu arestas.", mst_count );
